...
local logTag = "interactiveScreenInstance"
...
local function setupCustomModuleData(config)
  ...
  for k, v in pairs(mergedConfig) do
    ...
    if c and c.setupGaugeData and c.updateGaugeData then
      ...
      log("E", "ccfGauges.setupCustomModuleData", "Can't find controller: " .. k)
    end
  end
...
local function initSecondStage(jbeamData)
  ...
  if not configData then
    log("E", logTag..".initSecondStage", "Can't find config data...")
    return
  end
  ...
  if not gaugesScreenName then
    log("E", logTag..".initSecondStage", "Got no material name for the texture, can't display anything...")
    return
  else
    if htmlPath then
      ...
      screenManager.addScreen(gaugeHTMLTexture)
    else
      log("E", logTag..".initSecondStage", "Got no html path for the texture, can't display anything...")
      return
    end
  end
